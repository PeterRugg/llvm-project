...
#ifndef NDEBUG
    auto *Length =
        dyn_cast<ConstantInt>(cast<AnyMemTransferInst>(I)->getLength());
    assert((!Length || Length->getZExtValue() >= DL.getMaxPointerSize()) &&
           "Creating a must_preserve_tags copy with less than capability size");
#endif
